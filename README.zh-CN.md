...
- 开台第一页不带快照位置去问，Runtime 在页上说出的 `snapshot_ledger_position` 就是这一
  会话读的位置，即钉点。只有人按「跳到最新」或开启跟随（见下）时才会移动，每次同样用一张新的
  第一页来定；离线从不移动。之后每页都是
...
- 钉住之后，立刻在同一条连接上读一次 `status()` 和一次 `runtime_fact_snapshot()`，给实例卡
  「运行时实例」几行。状态只在人按「跳到最新」时重读；跟随时钉点每移动一次，只重读任务事实。头两行写两次读各自所在的序号；两者都可能
  晚于钉住的快照，所以这是开台时的状态，不是钉点上的状态。按契约，Runtime 会把这次状态读取本身记
...
- **从最新一端读起**：账本查询没有降序，所以时间线从钉住的位置往回按 256 个位置一窗读。一窗里的
  事件不会多于一页，一次查询就能整窗读完，各项过滤都由账本做。一直往回读，直到多出 256 行、读到
  位置 1、或已读 64 窗为止，行按**最新在上**排列；底部的「读更早」接着往下读。顶栏常驻已读窗口覆盖
  的位置（「已读序号 A–B」），有窗口的页说读取不完整时追加「源不完整」。
- **跳到最新、跟随最新（只在在线读面）**：「跳到最新」把钉点移到 Runtime 的最新位置，重读状态与
  事实（状态读取会在账本留一条观察事件），去掉时间上界，从新钉点重新读起。「跟随最新」先做同样的事
  但不读状态，之后每 2 秒用一张第一页问账本到了哪里：钉点动了，就把新旧钉点之间的窗口读到视图顶部
  ——已载入的行、选中项和正在读的帧都保留——并重读任务事实。页查询和事实快照都不写账本，所以跟随
  本身不产生事件。跟随中设了时间上界会停止跟随。离线没有在跑的 Runtime 写新事件，两个控件都不可用。
- **性能监视事件默认隐藏**：`perf.summary` 每 2 秒一条，会淹没别的事件，而账本查询不能排除某个
  模块。除非勾选「显示性能监视」或在模块框里选了性能监视，监控台在读每一窗时丢掉这些事件，在已载入
  条数旁写明丢了多少，并把性能监视留在模块列表里，仍可选中。这是唯一一处在监控台已拿到的行上做的
  过滤；等账本查询能排除事件类型，就改由查询来做。
...
- **时间上界**：滑块的起始位置就是它代表的状态——最右端即「全部」，第一次拖动是收窄。设了上界时，
  从不晚于它的最后一个位置读起——用单条读取二分查找，再多留一窗余量，因为账本时间不保证严格有序——
  实际显示什么仍由查询自己的时间上界决定。
...
- 读取在后台线程里做，每次请求带代号；慢读回来时若选中项已变就丢弃。**任何时候都不会
  显示过期的或未校验的图。**
- 读取由一条后台工作线程做，**同时只有一条**。两张读面的整份读都中途停不下来：被顶掉的那次
  读完为止（受 8 MiB 上限与 30 秒期限约束），结果直接丢弃；还在排队等工作线程时就被顶掉的，根本
  不开始。在线时这比监控台自己拼段、能在下一段之前停手时更费——Runtime 每一段都要把整份素材重新
  哈希一遍。
