...
- **从最新一端读起**：账本查询没有降序，所以时间线从钉住的位置往回按 256 个位置一窗读。一窗里的
  ...
  读一窗，读满 256 行、到位置 1、读满 16 窗或满 1 秒就停，以先到者为准。行按**最新在上**排列；底部
...
- **跳到最新、跟随最新（只在在线读面）**：「跳到最新」用一张新的第一页把钉点移到 Runtime 的最新位置，
  ...
  跟随。离线没有在跑的 Runtime 写新事件，两个控件都不可用。
...
- **时间上界**：滑块的起始位置就是它代表的状态——最右端即「全部」，第一次拖动是收窄。设了上界时，
  从上界估计落在的位置读起——按已提交的时间跨度、假设事件在时间上均匀来估，再多留两窗余量，不为此
  花任何读取——实际显示什么仍由查询自己的时间上界决定，顶栏写明实际读了哪些位置。上界与标签随手柄
  即时变化；手柄停住 0.4 秒后视图才重新读取。
