...
3. **实例**，可选，一进页面就开始查找：「跳过」让 `instances` 留空，之后点监控台顶栏的「实例配置」按钮添加；
   ...
   安装时已按 `SHA256SUMS` 核过；向导再核一次 sha256，在第一次查找时读取。**不要人填路径、网址或哈希。**大包里有
   ...
   「蔚蓝档案 / Blue Archive：日服 com.YostarJP.BlueArchive」（大包没给 `label` 时写 game id）——并写明是否发布件自带。
   只有发布件没带大包时，才用本机大包文件：填绝对路径，点「加入」，不要哈希；已有同一游戏的大包时拒收。每个勾选的实例
