...
3. **实例**，可选，一进页面就开始查找：「跳过」让 `instances` 留空，之后点监控台顶栏的「实例配置」按钮添加；
   ...
   资源是发布件自带的资源仓**大包**：`MEMBERS.json` 的 `bundles[]` 写明每个大包与它的 sha256，文件就在下载文件夹里，
   安装时已按 `SHA256SUMS` 核过；第一次查找时，向导要求每个大包以同一个 sha256 列在 `SHA256SUMS` 里，再核一次文件的
   sha256 后读取。哪一项不过，就在注意事项里写明那个大包，不提供它，其余照常提供。**不要人填路径、网址或哈希。**大包里有
   ...
   「蔚蓝档案 / Blue Archive：日服 com.YostarJP.BlueArchive」（大包没给 `label` 时写 game id）——写明是否发布件自带，
   声明了默认包却没有包名的服务器写明不可选。本机大包文件可以随时加入，主要用在发布件没带大包时：填绝对路径，点「加入」，
   不要哈希；已有同一游戏（不分大小写）的大包时拒收。每个勾选的实例
   填别名（默认 `mumu-<序号>`），并**各自选**「程序 · 服务器 · 包名」，每个声明了默认包的服务器一项；只有一项时替每个
   实例选上。向导不懂游戏，也不猜。「写入实例」把用到的每个大包只放一次：各资源包按 `bundle.json` 逐个核对后原样放到
   `<安装根>\packages\<game>\`；日志按实例写明程序、资源包的包 id、路径与 sha256；再为每个勾选的实例写一项——别名、新的
...
放到 `packages\<game>\` 的大包资源包、引导拉起的 Runtime 的日志，以及升级时的 `previous\` 之外，引导写的文件只有这一个。
