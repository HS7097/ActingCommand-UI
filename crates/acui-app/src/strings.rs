...
pub struct Labels {
    ...
    /// alias, instance_id, binding, application_id, capture_backend, touch_backend.
    pub form_fields: [&'static str; 6],
    /// The binding kind box, and the placeholders of instance_index,
    /// instance_name, host, port.
    pub binding_kinds: [&'static str; 3],
    pub binding_placeholders: [&'static str; 4],
    pub optional_note: &'static str,
    pub check_and_save: &'static str,
    pub entry_no_id: &'static str,
    /// field; field, value.
    pub value_missing: &'static str,
    pub value_not_u16: &'static str,
    /// error; instance_id.
...
pub const ZH: Labels = Labels {
    ...
    config_intro: "列出 actingd_config 的 instances；新增或编辑一项后「校验并保存」：先在同一目录写临时文件，交 actingd check-config 校验，通过才替换原文件；表单不管的字段原样保留。改动在 Runtime 重启后生效。",
    config_listed: "{} · 共 {} 个实例",
    ...
    add_instance: "新增实例",
    form_fields: ["alias（必填）", "instance_id", "绑定方式", "application_id", "capture_backend", "touch_backend"],
    binding_kinds: ["MuMu 序号 instance_index", "MuMu 名称 instance_name", "ADB 地址 host + port"],
    binding_placeholders: ["instance_index（0–65535）", "instance_name", "host", "port（0–65535）"],
    optional_note: "应用与两个后端选填，取值由 check-config 判定；留空就不写这一项。",
    check_and_save: "校验并保存",
    entry_no_id: "这一项没有字符串 instance_id，不能在这里编辑",
    value_missing: "{} 不能为空",
    value_not_u16: "{} 须为 0–65535 的整数：{}",
    ...
    saved_running: "已保存（check-config 通过）· Runtime 重启后生效 · 现在：{} · 在主窗口先「请求关闭」，停下后再「启动」",
    saved_stopped: "已保存（check-config 通过）· Runtime 重启后生效 · 现在：{} · 在主窗口点「启动」即按新配置运行",
};
...
pub const EN: Labels = Labels {
    ...
    config_intro: "Lists the instances of actingd_config. Add or edit one, then Check and Save: a temporary file is written in the same directory and checked by actingd check-config, and only an ok replaces the file; fields the form does not manage are kept. Changes take effect when the Runtime restarts.",
    ...
    form_fields: ["alias (required)", "instance_id", "Binding", "application_id", "capture_backend", "touch_backend"],
    binding_kinds: ["MuMu Index instance_index", "MuMu Name instance_name", "ADB Address host + port"],
    binding_placeholders: ["instance_index (0–65535)", "instance_name", "host", "port (0–65535)"],
    optional_note: "The app and the two backends are optional and check-config decides their valid values; an empty box writes nothing.",
    ...
    value_not_u16: "{} Must Be a Whole Number 0–65535: {}",
    ...
    saved_running: "Saved (check-config ok) · Takes Effect When the Runtime Restarts · Now: {} · In the Main Window, Request Shutdown, Then Start Once It Has Stopped",
    saved_stopped: "Saved (check-config ok) · Takes Effect When the Runtime Restarts · Now: {} · Start in the Main Window Runs the New Configuration",
};
